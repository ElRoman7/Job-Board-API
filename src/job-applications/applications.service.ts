...
async apply(applicationDto: ApplicationDto, user : User) {
    ...
    const findApplication = await this.findOneApplication(candidate.id, offerId);
    if (findApplication) {
  ...
  async findOneApplication(candidateId: string, offerId: string) : Promise<Application>{
